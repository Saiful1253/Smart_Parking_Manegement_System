...
import { MapPin, Car, X, Clock } from 'lucide-react';
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm rounded-xl p-3 shadow-lg z-[1000]">
          <p className="text-xs font-semibold text-gray-700 mb-2">Zone Status</p>
        ...
        {/* Admin Header */}
        <div className="absolute top-4 right-4 bg-slate-800/95 backdrop-blur-sm rounded-xl px-4 py-2 shadow-lg z-[1000]">
          <p className="text-white text-sm font-medium">Admin View - Live Parking Map</p>
        </div>
        ...
        {profile?.role === 'admin' && (
          <div className="absolute bottom-4 left-4 bg-blue-500/90 backdrop-blur-sm rounded-xl px-4 py-2 shadow-lg z-[1000]">
            <p className="text-white text-sm font-medium">Click on a marker to select zone and book for customers</p>
          </div>
        )}
      </div>
      ...
      {sidebarZone && (
        <div className="w-80 border-l border-gray-200 bg-white overflow-y-auto flex flex-col">
          <div className="p-4 border-b border-gray-100 bg-gradient-to-r from-slate-700 to-slate-800">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-lg font-bold text-white">{sidebarZone.name}</h3>
                <div className="flex items-center gap-1 mt-0.5">
                  <MapPin className="w-3.5 h-3.5 text-slate-300" />
                  <span className="text-sm text-slate-300">{sidebarZone.location}</span>
                </div>
              </div>
              <button
          ...
          <div className="p-4 flex-1">
            ...
            <div className="text-sm text-gray-600 space-y-2 mb-4">
              ...
              <div className="flex justify-between">
                <span>Type</span>
                <span className="font-medium text-gray-900">{typeLabel[sidebarZone.zone_type]}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-gray-100">
            <button
              onClick={() => setSelectedZone(sidebarZone)}
            ...
            >
              {sidebarZone.status !== 'active' ? 'Not Available' : freeSpots(sidebarZone) === 0 ? 'Zone Full' : 'Book for Customer'}
            </button>
          </div>
        </div>
      )}

      {/* Zone List when no zone is selected */}
      {!sidebarZone && (
        <div className="w-80 border-l border-gray-200 bg-gray-50 overflow-y-auto">
          <div className="p-4 border-b border-gray-200 bg-white">
            <h3 className="font-bold text-gray-900">All Parking Zones</h3>
            <p className="text-xs text-gray-500 mt-0.5">Click to view details and book</p>
          </div>
          <div className="p-2">
            {zones.map(zone => (
              <button
